...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x_w = self.n3(x).reshape(B, l, l, C).view(B * l, l, C)  
        ...
        x_w_q, x_w_k, x_w_v = self.ln1(x_w).reshape(B * l, l, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3,
                                                                                                              1,
                                                                                                              4)  
        x_ww = ((x_w_q @ x_w_k.transpose(-1, -2)) * self.scale).softmax(-1)  
        x_ww = (x_ww @ x_w_v)  
        ...
        x_h = self.n4(x).reshape(B, l, l, C).transpose(1, 2).contiguous().view(B * l, l, C)  

        x_h = x_h + self.pos2
        x_h_q, x_h_k, x_h_v = self.ln2(x_h).reshape(B * l, l, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3,
                                                                                                              1,
                                                                                                              4)  
        x_hh = ((x_h_q @ x_h_k.transpose(-1, -2)) * self.scale).softmax(-1)  
        x_hh = (
                    x_hh @ x_h_v)  
        ...
        out1 = (score1 @ x_w_v).transpose(1, 2).reshape(B * l, l, C)  
        out1 = out1.reshape(B, l, l, C).transpose(1, 2)  
        
        score2 = ((x_w_q @ x_h_k.transpose(-1, -2)) * self.scale).softmax(-1)
        out2 = (score2 @ x_h_v).transpose(1, 2).reshape(B * l, l, C)  
        out2 = out2.reshape(B, l, l, C)  
...
class CrossNet(nn.Module):
    ...
    def forward(self, x, y):
        ...
        b1 = self.stage1_cnn(b)  
        stage1 = self.stage1(stem)  
        
        stage2 = self.stage2(torch.cat([Token2image(stage1), self.c1x1_1(b1)], 1))  
        b2 = self.stage2_cnn(torch.cat([b1, self.c1x1_11(Token2image(stage1))], 1))  
        
        stage3 = self.stage3(torch.cat([Token2image(stage2), self.c1x1_2(b2)], 1))  
        b3 = self.stage3_cnn(torch.cat([b2, self.c1x1_22(Token2image(stage2))], 1))  
        
        stage4 = self.stage4(torch.cat([Token2image(stage3), self.c1x1_3(b3)], 1))  
        b4 = self.stage4_cnn(torch.cat([b3, self.c1x1_33(Token2image(stage3))], 1))  
        
        stage2_ = self.o1(stage2)  
        stage3_ = self.o2(stage3)  
        stage4_ = self.o3(stage4)  
        
        
        out1 = Token2image(stage2_)  
        ...
        out_cnn = torch.cat([out_cnn1, out_cnn2, out_cnn3], 1)  
        
        plus = self.outc1(out + out_cnn)  
        sub = self.outc2(out - out_cnn)  
...
if __name__ == "__main__":
    ...
    acabfnet = CrossNet(nclass=2, head=[4,8,16,32])
