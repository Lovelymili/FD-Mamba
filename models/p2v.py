...
from models.p2v_help.p2v_blocks import Conv1x1, Conv3x3, MaxPool2x2
...
class SimpleDecoder(nn.Module):
    def __init__(self, itm_ch, enc_chs, dec_chs, out_ch):
        super().__init__()
        ...
        self.conv_out = Conv1x1(dec_chs[-1], out_ch)
...
class P2VNet(nn.Module):
    def __init__(self, in_ch, out_ch, video_len=8, enc_chs_p=(32, 64, 128), enc_chs_v=(64, 128), dec_chs=(256, 128, 64, 32)):
        super().__init__()
        if video_len < 2:
            raise ValueError
        ...
        self.conv_out_v1 = Conv1x1(enc_chs_v[-1], out_ch)
        self.conv_out_v2 = Conv1x1(enc_chs_v[-1], out_ch)
        ...
        self.decoder1 = SimpleDecoder(enc_chs_p[-1], (2 * in_ch,) + enc_chs_p, dec_chs, out_ch)
        self.decoder2 = SimpleDecoder(enc_chs_p[-1], (2 * in_ch,) + enc_chs_p, dec_chs, out_ch)
    ...
    def forward(self, t1, t2):
        ...
        pred_t1 = self.decoder1(feats_p[-1], feats_p)
        pred_t2 = self.decoder2(feats_p[-1], feats_p)

        if self.return_aux:
            pred_v1 = self.conv_out_v1(feats_v[-1])
            pred_v1 = F.interpolate(pred_v1, size=pred_t1.shape[2:])
            pred_v2 = self.conv_out_v2(feats_v[-1])
            pred_v2 = F.interpolate(pred_v2, size=pred_t1.shape[2:])
            return pred_t1, pred_t2, pred_v1, pred_v2
        else:
