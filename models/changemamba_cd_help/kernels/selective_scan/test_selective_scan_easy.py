...
class SelectiveScanEasy(torch.autograd.Function):
    ...
    def backward(ctx, doys: torch.Tensor, *args):
        ...
        for i in chunks[::-1]:
            ...
            dhs = doys[i:i + chunksize].unsqueeze(4) * Cs[i:i + chunksize].unsqueeze(3) 
            dhs[-1] = dhs[-1] + dhprefix
            dhprefix = torch.einsum("lbgdn,lbgdn -> bgdn", dhs, Ats)
            dAts_hprefix = dhs * _hprefix.unsqueeze_(0) if i > 0 else None 
            ...
            if DEBUG:
                drAts_dtBus_div_rAts_v1 = d_dtBus_div_rAts * (dtBus / -(rAts * rAts)) 
...
def selective_scan_easyv2(us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, return_last_state=False, chunksize=64):
    ...
    def ss_chunk(us, dts, As, Bs, Cs, h0, mask):
        ...
        w_log = As[None, :, None, :, :] * (torch.cumsum(dts, dim=2)[..., None, :]) 
        v = us * dts 
        k = Bs 
        q = Cs 
        w = w_log.exp()
...
def selective_scan_easyv3(us, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=False, return_last_state=False, chunksize=64):
    ...
    _row_arange = _arange[None, :] 
    _col_arange = _arange[:, None] 
    ...
    def ss_chunk_h1y1(qs, ks, vs, ws=None, As=None, ts=None, dts=None, mask=None, scale=1):
        ...
        if ws is None:
            ...
            ws = torch.exp2(As[None, :, None, None, :, :] * _ts).flatten(0, 1) 
    ...
    def ss_chunk_h1y1_dk1(qs, ks, vs, ws=None, As=None, ts=None, dts=None, mask=None, scale=1):
        ...
        if ws is None:
            ...
            _ts = ts.view(-1, As.shape[0], *ts.shape[1:])[:, :, :, :, None, :]
            ws = (As[None, :, None, None, :, :] * _ts).exp().flatten(0, 1) 
        q_mul_w = qs[...,None] * ws * scale
    ...
    hts = ss_chunk_h(cws, ht1) 
    oys = ss_chunk_y(y1, hts, q_mul_w) 
    oys = oys.contiguous().view(B, G, -1, D)[:, :, :L, :].contiguous()
    hprefix = hts[:,-1,:,:].contiguous() 
